<?
	...
	function listMojib($db, $m_no, $m_type, $m_year, $use_tf, $del_tf, $search_field, $search_str, $nPage, $nRowCount) {

		$total_cnt = totalCntMojib($db, $m_no, $m_type, $m_year, $use_tf, $del_tf, $search_field, $search_str);
		...
		$query = "SELECT @rownum:= @rownum - 1  as rn, A.M_NO, A.M_TYPE, A.M_TITLE, A.M_YEAR, A.M_PDF, 
							...
							FROM TBL_MOJIB A, TBL_CODE_DETAIL B WHERE A.M_YEAR = B.DCODE AND B.PCODE = 'MOJIB_YEAR' ";
	...
	function totalCntMojib($db, $m_no, $m_type, $m_year, $use_tf, $del_tf, $search_field, $search_str){

		$query ="SELECT COUNT(*) CNT FROM TBL_MOJIB WHERE 1 = 1 ";
...
*/
	...
	function insertMojib($db, $arr_data) {
		...
		$query = "INSERT INTO TBL_MOJIB (".$set_field." REG_DATE) 
	...
	function insertMojibDetail($db, $arr_data) {
		...
		$query = "INSERT INTO TBL_MOJIB_DETAIL (".$set_field." REG_DATE) 
					values (".$set_value." now()); ";
	...
	function selectMojib($db, $m_no) {
		...
		$query = "SELECT * FROM TBL_MOJIB WHERE M_NO='$m_no' AND DEL_TF='N'";
	...
	function selectMojibDetail($db, $m_no){ //INPUT BOX의 갯수를 가지고 오기 위해
		...
		$query ="SELECT * FROM TBL_MOJIB_DETAIL WHERE M_NO='$m_no' AND DEL_TF = 'N' ORDER BY MD_NO ASC";
	...
	function updateMojib($db, $arr_data, $m_no) {
		...
		$query = "UPDATE TBL_MOJIB SET ".$set_query_str." ";
	...
	function updateMojibDetail($db, $arr_data, $md_no) {
		...
		$query = "UPDATE TBL_MOJIB_DETAIL SET ".$set_query_str." ";
...
//exit;
	...
	function deleteMojib($db, $adm_no, $m_no) {
		...
		$query = "UPDATE TBL_MOJIB SET DEL_TF = 'Y', DEL_ADM = '$adm_no', DEL_DATE = now() WHERE M_NO = '$m_no' ";
	...
	function deleteMojibDetail($db, $adm_no, $m_no) {
		...
		$query = "UPDATE TBL_MOJIB_DETAIL SET DEL_TF = 'Y', DEL_ADM = '$adm_no', DEL_DATE = now() WHERE M_NO = '$m_no' ";
	...
	function deleteMojibDetail_1($db, $adm_no, $md_no, $m_no) {
		...
		$query = "DELETE FROM TBL_MOJIB_DETAIL WHERE MD_NO = '$md_no' ";
		...
		if(!mysqli_query($db,$query)) {
			...
			echo "<script>document.location.href='mojib_write.php?mode=S&m_no=".$m_no."';</script>";
			exit;
		}
	}
	...
	function getMojibYear($db, $m_type) {
		...
		//$query = "SELECT DISTINCT M_YEAR FROM TBL_MOJIB WHERE M_TYPE = '$m_type' AND USE_TF = 'Y' AND DEL_TF = 'N' ORDER BY M_YEAR DESC ";
		
		$query = "SELECT DISTINCT A.M_YEAR
								FROM TBL_MOJIB A, TBL_CODE_DETAIL B
	...
	function getMojibInfo($db, $m_type, $m_year) {
		...
		$query = "SELECT * FROM TBL_MOJIB WHERE M_TYPE = '$m_type' AND M_YEAR = '$m_year' AND USE_TF = 'Y' AND DEL_TF = 'N' ORDER BY M_YEAR DESC LIMIT 1 ";
