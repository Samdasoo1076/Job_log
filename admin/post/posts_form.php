...
require __DIR__ . '/../com/biz/post.php';         // → admin/com/biz/post.php
require __DIR__ . '/../com/biz/folder.php';         // → admin/com/biz/post.php
$tree         = getFolders();
...
<form class="post-form" method="post" action="posts_form.php?folder_id=<?= $filterFolder ?>&page=<?= $page ?>" onsubmit="return submitContents(this);" novalidate>
...
<div class="form-group">
  ...
  <select name="folder_id" required>
    ...
    <?php
      function renderSelect(array $folders, string $prefix = '') {
        global $post;
        foreach ($folders as $f) {
          $sel = isset($post) && $post['folder_id']==$f['id'] ? 'selected' : '';
          echo "<option value=\"{$f['id']}\" $sel>"
             . $prefix . htmlspecialchars($f['name'])
             . "</option>";
          if ($f['children']) renderSelect($f['children'], $prefix . '-- ');
        }
      }
      renderSelect($tree);
    ?>
...
<div class="form-actions">
  ...
  <a href="posts_list.php?folder_id=<?= $filterFolder ?>&page=<?= $page ?>" class="btn cancel">Cancel</a>
</div>
</form>
