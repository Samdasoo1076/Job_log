...
<div class="pagination">
 <div class='board-page'>
    <? if ($page > 1) { ?>
        <button type='button' class='btn-page-first' disabled><span class='blind'>이전".$PBlock."개</span></button>
        <button type='button' class='btn-page-prev' disabled><span class='blind'>이전 페이지</span></button>
        <a href="?b=B_1_5&page=<?= $page - 1 ?>">&laquo; 이전</a>
    <? } ?>
    <? for ($i = 1; $i <= $total_pages; $i++) { ?>
        <a href="?b=B_1_5&page=<?= $i ?>" <?= $i === $page ? 'class="active"' : '' ?>><?= $i ?></a>
        <button type='button' class='btn-page-number is-current' aria-current='page' onClick='return false;'><?= $i?><span class='blind'>&nbsp;페이지</span></button>
    <? } ?>
    <? if ($page < $total_pages) { ?>
        <a href="?b=B_1_5&page=<?= $page + 1 ?>">다음 &raquo;</a>
        <button type='button' class='btn-page-next' disabled onClick='return false;'><span class='blind'>다음 페이지</span></button>
        <button type='button' class='btn-page-last' disabled><span class='blind'>다음".$PBlock."개</span></button>
    <? } ?>
    </div>
</div>
-->