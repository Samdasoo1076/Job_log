<?session_start();?>
<?
$_PAGE_NO = "21";
require $_SERVER['DOCUMENT_ROOT'] . "/_common/common_inc.php";
?>
<!DOCTYPE html>
<html lang="ko">

<head>
	<meta charset="utf-8" />
	<meta http-equiv="X-UA-Compatible" content="IE=edge" />
	<meta name="format-detection" content="telephone=no" />
	<meta name="viewport"
		content="width=device-width,initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no" />
	<meta name="keywords" content="원주미래산업진흥원" />
	<meta name="description" content="원주미래산업진흥원" />
	<title>공지사항 목록 | 원주미래산업진흥원</title>
	<link rel="stylesheet" type="text/css" href="../../assets/css/WFI.css" />

	<script src="../../assets/js/libs/jquery.min.js"></script>
	<script src="../../assets/js/libs/librurys.min.js"></script>
	<script src="../../assets/js/ui/ui.common.js"></script>
	<script src="../../assets/js/ui/ui.publish.js"></script><!-- 퍼블리싱 전용 (개발적용안함) -->

	<!-- 페이지 스크립트 영역 -->
	<script></script>
</head>

<body>
	<!-- 레이아웃 유형구분
		메인: main-wrapper,
		서브: sub-wrapper,
		기타: [기타]-wrapper
	 -->
	<div class="wrapper sub-wrapper">
		<!-- include_header.html -->
		<header class="header"></header>
		<!-- // include_header.html -->

		<!-- Container -->
		<main role="main" class="container">
			<!-- content -->
			<div id="content" class="content notice-list-page">
				<!-- content-header -->
				<div class="content-header">
					<!-- Sub Visual -->
					<div class="visual-wrap visual-02">
						<div class="inner">
							<h1>소통마당</h1>
							<div class="location">
								<span class="lc-home"><span class="blind">홈</span></span>
								<span class="lc-split"><span class="blind">&gt;</span></span>
								<span class="lc-cate"><span>소통마당</span></span>
								<span class="lc-split"><span class="blind">&gt;</span></span>
								<span class="lc-current" aria-current="page">공지사항</span>
							</div>
						</div>
					</div>
					<!-- // Sub Visual -->
					<!-- Sub Nav -->
					<div class="snb-wrap">
						<nav class="nav dep2-nav">
							<ul class="dep2-list">
								<li class="dep2-item">
									<a href="#" class="dep2-link is-current">공지사항</a>
								</li>
								<li class="dep2-item">
									<a href="#" class="dep2-link">공고안내</a>
								</li>
								<li class="dep2-item">
									<a href="#" class="dep2-link">보도자료</a>
								</li>
								<li class="dep2-item">
									<a href="#" class="dep2-link" aria-current="page">경영공시</a>
								</li>
							</ul>
						</nav>
					</div>
					<!-- // Sub Nav -->
				</div>
				<!-- // content-header -->
				<!-- content-body -->
				<div class="content-body">
					<!-- 게시판상세 페이지 -->
					<div class="board-view-page">
						<!-- 타이틀 영역 -->
						<div class="view-title-wrap">
							<h2 class="title">공지사항2024년도 11월 공지안내</h2>
							<div class="info">
								<span class="date">2024.11.01</span>
								<span class="form">원주미래산업진흥원</span>
							</div> 
						</div>
						<!-- // 타이틀 영역 -->
						<!-- 게시판상세 영역 -->
						<div class="board-view-wrap">
							<div class="board-view">
								<div class="board-view-cont">
									<div class="file-area">
										<div class="link-item"><a href="공지사항_안내_별첨문서_01_241101.pdf" download="">공지사항_안내_별첨문서_01_241101.pdf</a></div>
										<div class="link-item"><a href="공지사항_안내_별첨문서_01_241101.pdf" download="">공지사항_안내_별첨문서_01_241101.pdf</a></div>
										<div class="link-item"><a href="공지사항_안내_별첨문서_01_241101.pdf" download="">공지사항_안내_별첨문서_01_241101.pdf</a></div>
									</div>
									<div class="view-area">
										<div class="cont">
											본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 
본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 
본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 본문 게시글 영역입니다. 
										</div>
									</div>
									<div class="btn-list">
										<a href="#">목록</a>
									</div>
								</div>
							</div>
							<div class="board-view-page">
								<div class="pager">
									<span class="prev-view"><span class="txt">이전글</span></span>
									<a href="#">공지사항 이전글입니다. 공지사항 이전글입니다.</a>
									<span class="date">2024.11.01</span>
								</div>
								<div class="pager">
									<span class="next-view"><span class="txt">다음글</span></span>
									<a href="#">공지사항 이전글입니다. 공지사항 이전글입니다.</a>
									<span class="date">2024.11.01</span>
								</div>
								<!-- 글이 없을경우 -->
								<div class="pager disabled">
									<span class="next-view"><span class="txt">다음글</span></span>
									<a href="#" class="link">다음글이 없습니다.</a>
								</div>
								<!-- //글이 없을경우 -->
							</div>
						</div>
						<!-- // 게시판목록 영역 -->
					</div>
					<!-- // 게시판목록 페이지 -->
				</div>
				<!-- // content-body -->
			</div>
			<!-- // content -->
		</main>
		<!-- // Container -->

		<!-- include_footer.html -->
		<footer class="footer">
<?
	require "../_common/front_footer.php"; 
?>
		</footer>
		<!-- // include_footer.html -->
	</div>
</body>

</html>