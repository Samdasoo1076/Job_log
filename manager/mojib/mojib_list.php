...
#====================================================================
	...
	if ($mode == "D") {
		...
		for ($k = 0; $k < $row_cnt; $k++) {
			...
			$result_log = insertUserLog($conn, 'admin', $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], "모집요강 삭제 (".$tmp_idx.") ", "Delete");
		}
	}
...
#===============================================================
	...
	$nListCnt =totalCntMojib($conn, $m_no, $con_m_type, $con_m_year, $use_tf, $del_tf, $search_field, $search_str);
	...
	$arr_rs = listMojib($conn, $m_no, $con_m_type, $con_m_year, $use_tf, $del_tf, $search_field, $search_str, $nPage, $nPageSize);


	$result_log = insertUserLog($conn, "admin", $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], "모집 요강 리스트 조회", "List");
		...
		<form id="bbsList" name="frm" method="post">
			...
			<input type="hidden" name="use_tf" value="">
					...
					<div class="tbl_top">
						...
						<div class="right">
							<?= makeSelectBox($conn,"MOJIB","con_m_type",100,"선택","",$con_m_type)?>
							<select name="search_field" id="search_field">
								<option value="M_YEAR" <? if ($search_field == "M_YEAR") echo "selected"; ?> >모집년도</option>
							</select>
						...
						<table>
							...
							?>
									<tr>
										...
										<td><a href="javascript:js_view('<?=$M_NO?>');"><?=getDcodeName($conn, "MOJIB", $M_TYPE)?></a></td>
										<td><a href="javascript:js_view('<?=$M_NO?>');"><?=getDcodeName($conn, "MOJIB_YEAR", $M_YEAR)?></a></td>
										<td><a href="javascript:js_view('<?=$M_NO?>');"><?=$M_TITLE?></a></td>
										<td><a href="../../_common/new_download_file.php?menu=mojib&m_no=<?=$M_NO?>&field=m_pdf" target="_blank"><?=$M_PDF_NAME?></a></td>
										<td><a href="../../_common/new_download_file.php?menu=mojib&m_no=<?=$M_NO?>&field=m_hwp" target="_blank"><?=$M_HWP_NAME?></a></td>
