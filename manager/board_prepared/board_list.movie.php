		<colgroup>
			<col style="width:3%" />
			<col style="width:7%" />
			<col style="width:15%" /><!-- THUMB이미지 -->
			<col style="width:44%" />
			<col style="width:6%" /><!-- 작성자 -->
			<col style="width:10%" /><!-- 등록일 -->
			<col style="width:6%" /><!-- 메인여부 -->
	...
	<?
		...
		if (sizeof($arr_rs) > 0) {
			...
			for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
				...
				if ($LOGIN_TF == "Y") {
					$STR_LOGIN_TF = "<font color='navy'>사용</font>";
				} else {
					$STR_LOGIN_TF = "<font color='red'>사용안함</font>";
				}

				if ($USE_TF == "Y") {
					$STR_USE_TF = "<font color='navy'>노출</font>";
				} else {
					$STR_USE_TF = "<font color='red'>비노출</font>";
				}
				...
				if ($FILE_NM <> "") {
					$LINK01 = "/upload_data/board/".$FILE_NM;
				}
	?>
		<tr> 
			<td><input type="checkbox" name="chk[]" value="<?=$B_NO?>"></td>
			<td><a href="javascript:js_view('<?=$B_CODE?>','<?=$B_NO?>');"><?= $rn ?></a></td>
			<td><a href="<?=$HOMEPAGE?>" target="_blank"><img src="<?=$THUMB_IMG?>" width="200px"></a></td>
			...
			<td><?= $REG_DATE ?></td>
			<td class="filedown">
				<a href="javascript:js_toggle_tf('<?=$B_NO?>','<?=$USE_TF?>', 'USE');"><span><?=$STR_USE_TF?></span></a>
			</td>
			<td class="filedown">
				<?=number_format($HIT_CNT)?>
			</td>
			<td class="filedown">
				<?=$REF_IP?>
			</td>
