<colgroup>
	...
	<?
		...
		if (sizeof($arr_rs_top) > 0) {
			...
			for ($j = 0 ; $j < sizeof($arr_rs_top); $j++) {
				...
				$VIEW_TF				= trim($arr_rs_top[$j]["VIEW_TF"]);
	...
	?>
		<tr style="background: #dfdfdf"> 
			...
			<td style="text-align:left">
				<a href="javascript:js_view('<?=$B_CODE?>','<?=$B_NO?>','<?=$b_board_type?>');"><?=$TITLE?></a> <?=$reply_cnt?> <?=$str_lock?> <?=$str_file?>
			</td>
			<td><?= $WRITER_NM ?></td>
			<td><?= $REG_DATE ?></td>
			<td class="filedown">
	...
	?>
		<tr style="<?=$top_style?> <?=$view_style?>"> 
			...
			<?=$is_new?>
			<a href="javascript:js_view('<?=$B_CODE?>','<?=$B_NO?>');"><?=strip_tags($TITLE)?></a> <?=$reply_cnt?> <?=$str_lock?> <?=$str_file?></td>
			<td><?= $WRITER_NM ?></td>
			<td><?= $REG_DATE ?></td>
			<td class="filedown">
				<a href="javascript:js_toggle('<?=$B_NO?>','<?=$USE_TF?>');"><span><?=$STR_USE_TF?></span></a>
