...
#===============================================================
	...
	$nListCnt = totalCntBoard($conn, $b_code, $con_cate_01, $con_cate_02, $con_cate_03, $con_cate_04, $con_writer_id, $keyword, $con_reply_state, $con_use_tf, $del_tf, $search_field, $search_str);
	...
	$arr_rs = listBoard($conn, $b_code, $con_cate_01, $con_cate_02, $con_cate_03, $con_cate_04, $con_writer_id, $keyword, $con_reply_state, $con_use_tf, $del_tf, $search_field, $search_str, $nPage, $nPageSize, $nListCnt);

	$arr_rs_top = listBoardTop($conn, $b_code, $con_cate_01, $con_cate_02, $con_cate_03, $con_cate_04, $con_writer_id, $keyword, $con_reply_state, $con_use_tf, $del_tf, $search_field, $search_str, 0);

	$arr_rs_main = listBoardMain($conn, $b_code, $con_cate_01, $con_use_tf, $del_tf, '0');
...
function js_online_modify() {
	
	var mode = "ONLINE_UPDATE";
	var on_title	= $("#on_title").val();
	var on_info		= $("#on_info").val();

	var request = $.ajax({
		url:"/_common/ajax_board_dml.php",
		type:"POST",
		data:{mode:mode, on_title:on_title, on_info:on_info},
		dataType:"json"
	});
...
?>
				<div class="tit_h3"><h3><?=$p_menu_name ?> <?= $b_board_type?></h3></div>
...
<input type="hidden" name="nPageSize" value="<?=$nPageSize?>">
				...
				<div class="cont">
					...
					<?
							$arr_rs_online_info = selectOnline($conn, "1");

							$rs_on_title = "";
							$rs_on_info = "";

							if (is_array($arr_rs_online_info) && isset($arr_rs_online_info[0]) && isset($arr_rs_online_info[0]["ON_TITLE"])) {
								$rs_on_title	= trim($arr_rs_online_info[0]["ON_TITLE"]);
								$rs_on_info		= trim($arr_rs_online_info[0]["ON_INFO"]);
							}
					?>
					<!--
					<div class="tbl_style01 left">
						<table>
							...
							<tr>
								<th scope="row">온라인 설명회 제목</th>
								<td>
									<input type="text" name="on_title" id="on_title" value="<?=$rs_on_title?>" style="width: 90%;" />
								</td>
							</tr>
							<tr>
								<th scope="row">온라인 설명회 내용</th>
								<td>
									<textarea style="width: 90%; height:180px" name="on_info" id="on_info"><?=$rs_on_info?></textarea>
								</td>
							</tr>
					...
					<div class="btn_wrap">
						<button type="button" class="button" onClick="js_online_modify();">수정</button>
					</div>
					-->
					<? } ?>
					...
					<?	} else  if($b_board_type== "QNA"){?>
					<div class="tbl_top">
						<div class="left"><span class="txt">총 <span class="txt_c01"><?=number_format($nListCnt)?></span> 개</span></div>
						<div class="right">

							<div class="fl_wrap">
								<select name="con_reply_state" id="con_reply_state">
									<option value="ALL" <? if ($reply_state == "ALL") echo "selected"; ?> >답변 여부 전체</option>
									<option value="N" <? if ($reply_state == "N") echo "selected"; ?> >대기</option>
									<option value="Y" <? if ($reply_state == "Y") echo "selected"; ?> >완료</option>
								</select>
					...
					<div class="tbl_top">
						...
						<div class="right">
							...
							<div class="fl_wrap">
								...
								<? if ($b_board_cate) { ?>
								<?	if ($b_code == "B_1_7") { ?>
								<?=makeProgramBoardCodeSelectBox($conn, "con_cate_01", "전체", "", $b_board_cate, "style='width:200px'", $con_cate_01); ?>
								<?	} else { ?>
								<?=makeBoardCodeSelectBox($conn, "con_cate_01", "전체", "", $b_board_cate, "style='width:200px'", $con_cate_01); ?>	
								<?	} ?>
								<? } ?>
								<?= makeSelectBox($conn,"MA_TYPE","MA_TYPE","125","선택","", $con_cate_01)?>
								<select name="search_field" id="search_field">
									<option value="ALL" <? if ($search_field == "ALL") echo "selected"; ?> >전체</option>
									<option value="title" <? if ($search_field == "title") echo "selected"; ?> >제목</option>
									<option value="contents" <? if ($search_field == "contents") echo "selected"; ?> >내용</option>
...
<?
	$arr_rs = listBoard_faq($conn, $b_code, $con_cate_01, $con_cate_02, $con_cate_03, $con_cate_04, $con_writer_id, $keyword, $con_reply_state, $con_use_tf, $del_tf, $search_field, $search_str, $nPage, $nPageSize, $nListCnt);
...
<?
	require "board_list.movie.php";
?>
				<? } else if ($b_board_type == "QNA") { ?>
<?
	require "board_list.qna.php";
?>
				<? } else { ?>
<?
	require "board_list.board.php";
