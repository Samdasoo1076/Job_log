...
require_once $_SERVER['DOCUMENT_ROOT'] ."/_classes/com/util/AES2.php";
...
<head>
	...
	<style>
		...
		.replylist::-webkit-scrollbar-track {
		...
		.replylist fieldset .pic {
			float: left;
			margin: 0 20px 0 0
		}

		.replylist fieldset .pic span {
			display: block;
			width: 44px;
			height: 44px;
			overflow: hidden;
		...
		.replylist fieldset .pic span img {
			width: 100%;
			height: 100%
		}

		.replylist fieldset .pic strong {
			display: block;
			text-align: center;
			line-height: 30px;
			font-size: 14px;
		...
		.replylist ul li .btn-moreaction {
			position: absolute;
			right: 0;
			top: 30px;
			z-index: 20;
		...
		.replylist ul li .btn-moreaction:hover em {
			display: block
		}

		.replylist ul li .btn-moreaction button {
			width: 30px;
			height: 30px;
			background: url('../images/icon_moreaction.png') no-repeat 100% 30%;
			font-size: 0;
			text-indent: -99999rem
		}

		.replylist ul li .btn-moreaction em {
			display: none;
			position: absolute;
			right: 0;
			top: 20px;
		...
		.replylist ul li .btn-moreaction em a {
			display: block;
			text-align: center;
			color: #999;
			font-size: 13px;
		...
		.replylist ul li .btn-moreaction em a:hover {
			color: #010101
		}
	...
	<script language="javascript" type="text/javascript">
		...
		function js_answer(state) {
			...
			var frm = document.getElementById('answerFrm');
			var b_no = "<?= $b_no ?>";
						console.log(frm);
...
<body>
		...
		<div id="container">
			...
			<div class="contents">
				...
				<div class="cont">
						...
						<table>
							...
							<tbody>
								...
								<? if ($rs_cate_01) { ?>
									...
									} else if($b_code == "B_1_11") {
										$str_pcode_name = "QNA_TYPE";
								...
								<tr>
									...
									<th scope="row">이메일</th>
									<td><?= decrypt($key, $iv, $rs_email)?> </td>
								</tr>

								<tr>
									<th scope="row">게시 기간</th>
									<td colspan="3"><? if ($rs_date_use_tf == "Y") { ?><?= $rs_start_date ?> ~
											<?= $rs_end_date ?><? } else { ?>무제한 노출<? } ?></td>
								</tr>
								...
								<? if ($b_board_type == "RATE") { ?>
									...
									<tr>
										...
										<td colspan="3">
											<? if ($rs_file_rnm) { ?>
												<a
													href="/_common/new_download_file.php?menu=board&b_code=<?= $rs_b_code ?>&b_no=<?= $rs_b_no ?>&field=file_nm"><?= $rs_file_rnm ?></a>
											<? } ?>
										</td>
									</tr>
				 ...
				 <? if ($b_code == "B_1_11") { ?>
					<form id="answerFrm" name="frm" method="post" enctype="multipart/form-data">
				...
				<div class="cont">
								...
								<th scope="row">답변</th>
									<td colspan="3">
										<span class="fl" style="padding-left:0px;width:90%;height:500px;">
												<textarea name="reply" id="reply"  style="padding-left:0px;width:100%;height:450px;"><?=$rs_reply?></textarea>
											</span>
									</td>
								</tr>
							</tbody>
					...
					<div class="btn_wrap">
					<? if($b_code == "B_1_11") {?>
						<button type="button" class="button" onClick="js_answer('Y')">답변 등록</button>
						<? }?>
											...
											<td colspan="3" style="padding: 10px 10px 10px 15px">
												<span class="fl" style="padding-left:0px;width:940px;height:400px;">
													<textarea name="reply" id="reply"
														style="padding-left: 0px; width: 930px; height: 350px; display: none;"><?= $rs_reply ?></textarea>
												</span>
											</td>
										</tr>
									</tbody>
					...
					<div class="replylist">
						<form name="frm_comment" method="get">
							...
							<fieldset>
								...
								<span></span>
								<p><textarea cols="" rows="" placeholder="댓글을 입력해주세요." name="contents"
										id="contents"></textarea><button type="button"
										onClick="js_comment_save();">등록</button></p>
							</fieldset>
						</form>

						<ul id="div_recomm_list">
							<li>
								<span class="pic"><img src="/upload_data/profile/20181212135754_3.jpg" style="width:44px"
										alt=""></span>
								<p>저는 조금 늦을 것 같습니다. 8시까지는 가겠습니다.</p>
								<span>2018.10.10 &nbsp;&nbsp;&nbsp;&nbsp; 홍길동</span>
								<span class="btn-moreaction"><button type="button">수정/삭제</button><em><a href="#">수정</a><a
											href="#">삭제</a></em></span>
							</li>
							<li>
								<span class="pic"><img src="/upload_data/profile/20181212135754_3.jpg" style="width:44px"
	...
	<iframe src="" name="ifr_hidden" frameborder="no" width="0" height="0" marginwidth="0" marginheight="0"
		border="0"></iframe>
</body>
