...
#====================================================================
	...
	$ea_no					= isset($_POST["ea_no"]) && $_POST["ea_no"] !== '' ? $_POST["ea_no"] : (isset($_GET["ea_no"]) ? $_GET["ea_no"] : '');
	$apply_tf				= isset($_POST["apply_tf"]) && $_POST["apply_tf"] !== '' ? $_POST["apply_tf"] : (isset($_GET["apply_tf"]) ? $_GET["apply_tf"] : '');
	$con_apply_tf		= isset($_POST["con_apply_tf"]) && $_POST["con_apply_tf"] !== '' ? $_POST["con_apply_tf"] : (isset($_GET["con_apply_tf"]) ? $_GET["con_apply_tf"] : '');
	$apply_all_txt	= isset($_POST["apply_all_txt"]) && $_POST["apply_all_txt"] !== '' ? $_POST["apply_all_txt"] : (isset($_GET["apply_all_txt"]) ? $_GET["apply_all_txt"] : '');
	...
	if ($mode == "T") {
		updateEnterInfoApplyProcessTF($conn, $apply_tf, $_SESSION['s_adm_no'], $ea_no);
	}

	if ($mode == "TA") { //신청 일괄처리여부
		updateEnterInfoApplyProcessTFAll($conn, $apply_tf, $_SESSION['s_adm_no'], $chk);
	}
...
#===============================================================
	...
	$arr_rs = listEnterInfoApply($conn, $ea_no, $ea_name, $ea_who, $ea_who_year, $ea_post, $ea_addr, $ea_addr_detail, $ea_phone, $ea_pwd, $agree_tf, $con_apply_tf, $use_tf, $del_tf, $f, $s, $nPage, $nPageSize, $nListCnt);
...
function js_toggle(ea_no, use_tf) {
	...
	function js_excel_print() {
		...
		frm.method = "post";
		frm.target = "";
		frm.action = "enterinfoapply_excel_list.php";
		frm.submit();
	}
			...
			<input type="hidden" name="nPageSize" value="<?=$nPageSize?>">
					...
					<div class="tbl_top">
						...
						<div class="right">
							...
							<button type="button" class="button" onClick="document.location='/manager/enter/enterinfo_list.php'">신청자료 관리</button>
						</div>
					</div>
						...
						<table>
							...
							<?
								...
								if (sizeof($arr_rs) > 0) {
									...
									for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
										...
										if ($APPLY_TF == "N") {
											$STR_APPLY_TF = "<font color='red'>접수</font>";
										} else{
											$STR_APPLY_TF = "<font color='blue'>발송</font>";
										}
							...
							?>
										<tr>
											...
											<td style="text-align:left"><?=$EA_MEMO?></td>
											<td><?=$EA_PHONE?></td>
											<td><?=$EA_POST?></td>
											<td style="text-align:left"><?=$EA_ADDR?> <?=$EA_ADDR_DETAIL?></td>
					...
					<div class="btn_wrap">
						<div class="right">
							...
							<? if ($sPageRight_I == "Y") { ?>
								<button type="button" class="button" onClick="javascript:js_toggle_apply_all();">발송 처리</button>
							<? } ?>
						...
						<?
							...
							if (sizeof($arr_rs) > 0) {
								...
								$strParam = $strParam."&con_apply_tf=".$con_apply_tf."&con_confirm_tf=".$con_confirm_tf;
						?>
