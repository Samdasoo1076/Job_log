...
# Confirm right
#==============================================================================
	$menu_right = "SH001"; // 메뉴마다 셋팅 해 주어야 합니다
...
#====================================================================
	...
	$arr_rs_detail = array();
#====================================================================
	$savedir1 = $g_physical_path."upload_data/major";
#====================================================================
...
*/
		...
		$result_log = insertUserLog($conn, 'admin', $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], '학과 및 전공 수정 (제목 : '.$m_type.'-'.$m_year.') ', 'Update');
	}

	if ($mode == "D") {
		$result = deleteMojib($conn, $_SESSION['s_adm_no'], (int)$m_no);
		$result = deleteMojibDetail($conn, $_SESSION['s_adm_no'], (int)$m_no);
		$result_log = insertUserLog($conn, 'admin', $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], '학과 및 전공 삭제 처리 (제목 : '.$m_type.'-'.$m_year.') ', 'Delete');
	}

	if ($mode == "ITEM") {
		$result = deleteMojibDetail_1($conn, $_SESSION['s_adm_no'], (int)$item_no, (int)$m_no);
		$mode = "S";
		$result_log = insertUserLog($conn, 'admin', $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], '학과 및 전공 항목 삭제 처리 (제목 : '.$m_type.'-'.$m_year.') ', 'Delete');
	}
	...
	if ($mode == "S") {
		...
		$result_log							= insertUserLog($conn, 'admin', $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], '학과 및 전공 조회 (제목 : '.$rs_m_type.'-'.$rs_m_year.') ', 'Read');
...
<script language="javascript">
		alert('정상 처리 되었습니다.');
		document.location.href = "major_list.php<?=$strParam?>";
</script>
</head>
</html>
<?
...
<input type="hidden" name="search_str" value="<?=$search_str?>">
				...
				<div class="cont">
					<div class="tit_h4 first"><h4>학과 및 전공</h4></div>
					<div class="tbl_style01 left">
						<table>
							...
							<tbody>
								...
								<tr>
									<th>학년도 <img src="../images/img_essen.gif" alt="필수입력" /></th>
									<td colspan="3">
										<input type="hidden" name="m_type" id="m_type" value="MAJOR">
										<?= makeSelectBox($conn,"MAJOR_YEAR","m_year",100,"선택","",$rs_m_year)?>
									</td>
								</tr>
								<tr>
									<th>제목</th>
								...
								<tr>
									...
									?>
										<span class="tbl_txt"><a href="../../_common/new_download_file.php?menu=major&m_no=<?=$rs_m_no?>&field=m_pdf" target="_blank"><?=$rs_m_pdf_name?></a></span>
										&nbsp;&nbsp;
