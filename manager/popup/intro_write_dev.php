...
$rs_e_min = "";
    ...
    if (isset($_FILES['file_nm']) && $_FILES['file_nm']['error'] === UPLOAD_ERR_OK) {
        $allowed_extensions = ['jpg', 'png', 'gif', 'jpeg', 'tif']; // 허용되는 파일 확장자
        $max_size = 4; // 업로드 파일 최대 크기 (MB)

        // upload 함수 사용
        $file_nm = upload($_FILES['file_nm'], $upload_dir, $max_size, $allowed_extensions);
        if ($file_nm !== false) {
    ...
    $arr_data = array(
        ...
        "FILE_RNM" => $file_rnm,
        "FILE_PATH" => $file_path,
        "FILE_SIZE" => $_FILES['file_nm']['size'] ?? 0,
        "USE_TF" => $use_tf,
    ...
    if ($new_intro_no) {
        insertUserLog($conn, 'admin', $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], '인트로 등록 (제목 : ' . $intro_title . ')', 'Insert');
        echo "<script>alert('인트로가 성공적으로 등록되었습니다.');</script>";
    } else {
