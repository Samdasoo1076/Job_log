<script>
	document.addEventListener('DOMContentLoaded', function () {
		// 버튼과 입력 필드 가져오기
		const searchButton = document.getElementById('btn_search_header');
		const searchInput = document.getElementById('input_search_header');
		...
		if (searchButton && searchInput && searchForm) {
			...
			console.error('검색 버튼, 입력 필드, 또는 폼을 찾을 수 없습니다.');
		}
	})
			...
			<div class="snb-wrap">
				<ul class="util pc">
					<li>
						<? if (!empty($_SESSION['m_id'])): ?>
							<div class="btn-user-wrap">
								<a href="#" class="btn-user dropdown-toggle" type="button" data-bs-toggle="dropdown"
									aria-expanded="false">
									<span class="name"><?= $_SESSION['m_id'] ?></span> 님
								</a>
								<div class="user-menu dropdown-menu">
									<ul class="user-menu-list">
										<li><a href="../member/mypage_modify.php">회원정보 수정</a></li>
										<li><a href="../member/mypage.php">회원정보</a></li>
									</ul>
								</div>
							</div>
						<? endif; ?>
					...
					<li>
						<? if (!empty($_SESSION['m_id'])): ?>
							...
							<div class="btn-log-wrap on">
								<a href="../member/logout.php" class="btn-log"><span class="blind">사용자메뉴</span></a>
							</div>
						<? else: ?>
							<!-- 비로그인 상태 -->
							<div class="btn-log-wrap">
								<a href="../member/login.php" class="btn-log"><span class="blind">로그인</span></a>
							</div>
						<? endif; ?>
					...
					<li>
						...
						<div class="header-search">
							<div class="search_wrap">
								...
								<div class="input">
									<input id="input_search_header" type="search" name="searchKeyword"
										placeholder="검색어를 입력하세요">
									<!-- <span class="delete"><span class="blind">delete</span></span>  -->
									<button id="btn_search_header" type="submit" class="btn_search"><span
					...
					<li>
						...
						<a href="javascript:void(0)" class="btn-gnb" data-bs-toggle="modal"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="mem-area">
					<? if (!empty($_SESSION['m_id'])): ?>
						<!-- 로그인 상태 -->
						<h3><?= $_SESSION['m_id'] ?>님</h3>
						<div class="mem-area-btn-wrap">
							<button type="button" class="ui-btn logout">
								<span><a href="../member/logout.php">로그아웃</a></span>
							</button>
						</div>
					<? else: ?>
						<!-- 비로그인 상태 -->
						...
						<div class="mem-area-btn-wrap">
							...
							<button type="button" class="ui-btn join">
								<span><a href="../member/member_form.php">회원가입</a></span>
							</button>
						</div>
					<? endif; ?>
				</div>
							...
							if (sizeof($arr_menu_rs_01) > 0) {
								...
								for ($j = 0; $j < sizeof($arr_menu_rs_01); $j++) {
									...
									<li class="dep1-item">
										...
										<button type="button" class="dep2-open" data-bs-toggle="collapse"
											data-bs-target="#dep2-<?= ($j + 1) ?>" aria-controls="dep2-<?= ($j + 1) ?>">
											<span class="blind">열기</span>
										</button>
										<div class="dep2-list-wrap" id="dep2-<?= ($j + 1) ?>"
						...
						<div class="login-menu">
							<ul class="login-list">
								<?php if (!empty($_SESSION['m_no'])): ?>
									<!-- 로그인 상태 -->
									<li>
										<a href="#"><span class="user-name"><?= $_SESSION['m_id'] ?></span>님</a>
									</li>
									<li>
										<a href="../member/logout.php" class="logout">로그아웃</a>
									</li>
								<?php else: ?>
									<!-- 비로그인 상태 -->
									<li>
