...
#====================================================================
require_once $_SERVER['DOCUMENT_ROOT'] . "/_classes/com/db/DBUtil.php";
...
if (isset($m) && $m == "") {
	$m = isset($_POST["m"]) && $_POST["m"] !== '' ? $_POST["m"] : (isset($_GET["m"]) ? $_GET["m"] : '');
}

$l = isset($_POST["l"]) && $_POST["l"] !== '' ? $_POST["l"] : (isset($_GET["l"]) ? $_GET["l"] : '');
$p = isset($_POST["p"]) && $_POST["p"] !== '' ? $_POST["p"] : (isset($_GET["p"]) ? $_GET["p"] : '');
$bn = isset($_POST["bn"]) && $_POST["bn"] !== '' ? $_POST["bn"] : (isset($_GET["bn"]) ? $_GET["bn"] : '');
$cn = isset($_POST["cn"]) && $_POST["cn"] !== '' ? $_POST["cn"] : (isset($_GET["cn"]) ? $_GET["cn"] : '');
...
$ref_ip = isset($_POST["ref_ip"]) && $_POST["ref_ip"] !== '' ? $_POST["ref_ip"] : (isset($_GET["ref_ip"]) ? $_GET["ref_ip"] : '');
...
require_once $_SERVER['DOCUMENT_ROOT'] . "/_classes/com/util/Util.php";
require_once $_SERVER['DOCUMENT_ROOT'] . "/_classes/com/util/AES2.php";
require_once $_SERVER['DOCUMENT_ROOT'] . "/_classes/com/etc/etc.php";
require_once $_SERVER['DOCUMENT_ROOT'] . "/_classes/biz/page/page.php";
require_once $_SERVER['DOCUMENT_ROOT'] . "/_classes/biz/config/config.php";
...
if (($this_date < $config_briefing_start) || ($this_date > $config_briefing_end))
	$config_briefing_tf = "N";
if (($this_date < $config_tour_start) || ($this_date > $config_tour_end))
	$config_tour_tf = "N";

if ($this_date > $config_briefing_start)
	$config_briefing_start = $next_date;
if ($this_date > $config_tour_start)
	$config_tour_start = $next_date;
...
if (isset($b) && $b <> "") {
	...
	if (isset($m) && $m == "read") {
		...
		// 최근 업데이트 ISO 날짜
		if ($rs_iso_up_date == "")
			$rs_iso_up_date = $rs_iso_reg_date;
...
# common head 
#=====================================================================
require $_SERVER['DOCUMENT_ROOT'] . "/_common/front_head.php";
require $_SERVER['DOCUMENT_ROOT'] . "/_common/front_header_dev.php";
